...
local GC_RUN = os.getenv('GC')
local GC_INFO = os.getenv('GC_INFO')
...
Environment:
  GC                      Force collectgarbage() after each test case
  GC_INFO                 Display collectgarbage('count') after each test case
]]
...
TextOutput.__class__ = 'TextOutput'
    ...
    function TextOutput:endTest( node )
        if node:isPassed() then
            if self.verbosity > M.VERBOSITY_DEFAULT then
                if GC_INFO then
                  io.stdout:write(string.format("Ok %3.0f\n",collectgarbage('count')))
                else
                  io.stdout:write("Ok\n")
                end
            else
        ...
        if GC_RUN then collectgarbage() end
    end
...
if EXPORT_ASSERT_TO_GLOBALS then
    ...
    function M.LuaUnit.isMethodTestName( s )
        ...
        return string.sub(s, 1, 4):lower() == 'test' or string.sub(s, 1, 3):lower() == 'try'
    end
    ...
    function M.LuaUnit.parseCmdLine( cmdLine )
        ...
        local function parseOption( option )
            ...
            elseif option == '--version' then
                result['version'] = true
                return
            elseif option == '--verbose' or option == '-v' or option == '-vv' then
...
----------------------------------------------------------------
    ...
    function M.LuaUnit.statusLine(result)
        ...
        if result.notImplementedCount > 0 then
            table.insert(s, string.format("%d failed tries", result.notImplementedCount))
        end
    ...
    function M.LuaUnit:startTest( testName, methodName  )
        ...
        self.result.currentNode.methodName = methodName
    ...
    function M.LuaUnit:addStatus( err )
        ...
        local isTry = string.sub(node.methodName, 1, 3):lower() == 'try'
        if string.sub(node.methodName, 1, 3):lower() == 'try' then
          if err.status == NodeStatus.FAIL then
            self.result.notImplementedCount = self.result.notImplementedCount+1
            node:fail( err.msg, err.trace )
            self.output:addStatus( node )
            err.status = NodeStatus.PASS
            --node:pass()
            return
          end
        end
    ...
    function M.LuaUnit:execOneFunction(className, methodName, classInstance, methodInstance)
        ...
        self:startTest(prettyFuncName,methodName)
